...
for j in tqdm(range(npix)):
    for i in range(npix):
        ...
        pixel_grid[j, i] = 1
        convolved = gaussian_filter(pixel_grid, sigma=4.0)  # * mask
...
# n1, n2 = 35, 45
# img = get_dist(f"{output_path}implane.npy", "01373_017_02_04_4_F430M")
# img = gaussian_filter(img, sigma=0)
# img = img[n1 : n1 + source_size, n2 : n2 + source_size]
# plt.imshow(img)
# plt.colorbar()
# plt.show()
...
class ImageBasis(zdx.Base):
    ...
    discard_first: bool = eqx.field(static=True)

    def __init__(self, basis_dict: dict, n_basis: int, discard_first: bool = False):
        self.n_basis = n_basis - discard_first
    ...
    def to_eigenbasis(self, img: Array) -> Array:
        # TODO do I need np.linear.pinv here instead?
        # return np.dot(self.M.T, img.flatten())  # shape (n_basis,)
    ...
    def from_eigenbasis(self, coeffs: Array) -> Array:
        ...
        #     coeffs = np.concat((np.array([-350]), coeffs))
        return np.dot(self.M, coeffs).reshape((self.size, self.size))
...
# %%
tukey_basis = ImageBasis(basis_dict, n_basis=200)
...
def loss_fn(model, exp, args={"reg_dict": {}}):
    ...
    # prior on the sum
    total_sum = model.get_distribution(exp).sum()
...
def scheduler(lr: float, warmup: int):

    return optax.warmup_constant_schedule(0, lr, warmup_steps=warmup)
...
def warmup_adam(lr: float, warmup: int):
    """
    ...
    """
    return _base_adam(scheduler(lr, warmup))
